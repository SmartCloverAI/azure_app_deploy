...
def preparing_inference_df(request_inference_data, training_columns):
    ...
    inference_data_df['DATA_LIVRARE'] = pd.to_datetime(inference_data_df['DATA_LIVRARE'], errors='coerce')
    inference_data_df['year_DATA_LIVRARE'] = inference_data_df['DATA_LIVRARE'].dt.year
    inference_data_df['month_DATA_LIVRARE'] = inference_data_df['DATA_LIVRARE'].dt.month
    inference_data_df['week_DATA_LIVRARE'] = inference_data_df['DATA_LIVRARE'].dt.day_of_week
    inference_data_df['day_DATA_LIVRARE'] = inference_data_df['DATA_LIVRARE'].dt.day

    inference_data_df = inference_data_df.drop(columns = ['DATA_LIVRARE'])
    inference_data_df = inference_data_df.drop(columns = ['DATA_SCADENTA'])
    inference_data_df = inference_data_df.drop(columns = ['INFO_SUPLIMENTARE'])
...
def init():
    ...
    root = "models"
    model_folder = "tf_anomaly_facturi_export"

    model_file = "autoencoder_DOCUMENTE_FACTURI_EXPORT.keras"
